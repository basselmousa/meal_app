import 'package:flutter/material.dart';
import 'package:shared_preferences/shared_preferences.dart';

class ThemeProvider extends ChangeNotifier {
  var primaryColor = Colors.pink;
  var accentColor = Colors.amber;

  var tm = ThemeMode.system;
  String themeText = "s";

  onChenged(color, n) async {
    n == 1
        ? primaryColor = _setMaterialColor(color.hashCode)
        : accentColor = _setMaterialColor(color.hashCode);
    notifyListeners();
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    sharedPreferences.setInt("primaryColor", primaryColor.value);
    sharedPreferences.setInt("accentColor", accentColor.value);
  }

  MaterialColor _setMaterialColor(int colorVal) {
    return MaterialColor(
      colorVal,
      <int, Color>{
        50: Color(0xFFFCE4EC),
        100: Color(0xFFF8BBD0),
        200: Color(0xFFF48FB1),
        300: Color(0xFFF06292),
        400: Color(0xFFEC407A),
        500: Color(colorVal),
        600: Color(0xFFD81B60),
        700: Color(0xFFC2185B),
        800: Color(0xFFAD1457),
        900: Color(0xFF880E4F),
      },
    );
  }

  getThemeColors() async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    primaryColor = _setMaterialColor(sharedPreferences.getInt("primaryColor") ?? 0xFFE91E63);
    accentColor = _setMaterialColor(sharedPreferences.getInt("accentColor") ?? 0xFFFFC107);
    notifyListeners();
  }

  void themeModeChanged(newThemeVal) async {
    tm = newThemeVal;
    _getThemeText(tm);
    notifyListeners();

    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    sharedPreferences.setString("textTheme", themeText);
    print(sharedPreferences.getString("textTheme"));
  }

  void _getThemeText(ThemeMode tm) {
    if (tm == ThemeMode.light) {
      themeText = "l";
    } else if (tm == ThemeMode.dark) {
      themeText = "d";
    } else if (tm == ThemeMode.system) {
      themeText = "s";
    }
  }

  getThemeMode() async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    themeText = sharedPreferences.getString("textTheme") ?? 's';
    if (themeText == "l") {
      tm = ThemeMode.light;
    } else if (themeText == "d") {
      tm = ThemeMode.dark;
    } else if (themeText == "s") {
      tm = ThemeMode.system;
    }
    notifyListeners();
  }
}
